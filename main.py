...
if __name__ == "__main__":
    ...
    g.showDirectedGraph()  # 绘制图形 —— 需求2

    print('test3: please input 2 words:\n')
    word1 = input('word1:\n')
    word2 = input('word2:\n')
    g.queryBridgeWords(word1, word2)  # 输出桥接词 —— 需求3
    print()

    sentence = input('test4:input setence:\n')
    print(g.generateNewText(sentence))
    print()

    print('输入两个节点，求最短路径')   # 输出最短路径 —— 需求5
    node1 = input('node1:')
    node2 = input('node2:')
    shortest_path=g.calcShortestPath(node1, node2)
    print(shortest_path)  
    if(shortest_path != None):
        g.print_calcShortestPath(shortest_path)

    var = input('展示所有点到一个节点的最短距离，请输入节点信息：')
    for node in g.get_nodes():
        print(g.calcShortestPath(var, node))
    print()

    r = random.choice(g.get_nodes())
    print(g.randomWalk(r))  # 随机游走 —— 需求6
